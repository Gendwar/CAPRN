...
class DFIBlock(nn.Module):
    def __init__(self, in_channels, channels):
        ...
        self.conv = nn.Conv2d(in_channels, 2, 3, 1, 1)
...
class LAT(nn.Module):
    def __init__(self, dim, num_heads, kernel_size=3, qk_scale=None, use_rpb=True):
        ...
        if use_rpb:
            self.rpb = nn.Parameter(torch.zeros(self.num_heads, self.rpb_size, self.rpb_size))
        vectors = [torch.arange(-s // 2 + 1, s // 2 + 1) for s in [kernel_size] * 2]
        grids = torch.meshgrid(vectors)
        grid = torch.stack(grids, -1).type(torch.FloatTensor)
        v = grid.reshape(self.kernel_size ** 2, 2)
        self.register_buffer('v', v)


    def apply_pb(self, attn, N):
        # attn: B, N, self.num_heads, 1, tokens = (3x3x3)
        bias_idx = torch.arange(self.rpb_size**2).unsqueeze(-1).repeat(N, 1)
        return attn + self.rpb.flatten(1,2)[:, bias_idx].reshape(self.num_heads, N, 1, self.rpb_size**2).transpose(0,1)
    ...
    def forward(self, q, k, return_attn_matrix=False):
        ...
        Q = q.reshape(B, H, W,  self.num_heads, C // self.num_heads).permute(0, 3, 1, 2, 4) * self.scale
        ...
        k = F.pad(k, (pdr, pdr, pdr, pdr))  # 1, C, H+2, W+2
        if return_attn_matrix:
            K = k.reshape(B, self.num_heads, C // self.num_heads, H + pd, W + pd).permute(0, 1, 3, 4, 2)
        k = k.flatten(0, 1)  # BxC, H+2, W+2
        k = k.unfold(1, self.kernel_size, 1).unfold(2, self.kernel_size, 1).permute(0, 3, 4,  1, 2)  # C, 3, 3, 3, H, W, T
        ...
        x = (attn @ self.v)  # B x N x heads x 1 x 2
        x = x.reshape(B, H, W, self.num_heads * 2).permute(0, 3, 1, 2)
        if return_attn_matrix:
            Q = F.normalize(Q.squeeze(0).flatten(1), dim=0)
            QQT = Q @ Q.transpose(0, 1)
            K = F.normalize(K.squeeze(0).flatten(1), dim=0)
            KKT = K @ K.transpose(0, 1)
            return x, F.normalize(QQT), F.normalize(KKT)
        else:
            return x
...
class PAN(nn.Module):
    def __init__(self,
        ...
        self.dfi1 = DFIBlock(2 * num_heads[4], 3 * 2 * num_heads[4])
        ...
        self.dfi2 = DFIBlock(2 * num_heads[3], 3 * 2 * num_heads[3])
        ...
        self.dfi3 = DFIBlock(2 * num_heads[2], 3 * num_heads[2] * 2)
        ...
        self.dfi4 = DFIBlock(2 * num_heads[1], 3 * num_heads[1] * 2)
        ...
        self.dfi5 = DFIBlock(2 * num_heads[0], 3 * num_heads[0] * 2)
    ...
    def forward(self, fixed, moving):
        ...
        #q,k size =[B,W,H,headdim*headnum]  
        w = self.lat5(q5, k5)
        w = self.dfi5(w)
